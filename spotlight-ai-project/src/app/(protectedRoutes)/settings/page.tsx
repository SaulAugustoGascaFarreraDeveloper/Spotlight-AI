...
import { Check, LucideAlertCircle, LucideArrowRight, LucideCheckCircle2 } from 'lucide-react'
...
const SettingsPage = async () => {
    ...
    const isConnected = !!checkUser?.user?.stripeCustomerId

    const stripeLink = getStripeOAuthLink('api/stripe-connect', checkUser.user.id)
    ...
    <div className='w-full mx-auto py-8 px-4'>
      ...
      <div className="w-full p-6 border border-input rounded-lg bg-background shadow-sm">
                <div className="flex items-center mb-4 gap-6">
                    ...
                    <div>
                        <h2 className='text-xl font-semibold text-primary'>Stipe Connect</h2>
                ...
                <div className='my-6 p-4 bg-muted rounded-md'>
                    <div className="flex items-start">
                        ...
                        <div>
                            <p>
                                {isConnected ? 'Your Stripe account is connected' : 'Your Stripe account is not connected yet'}
                            </p>
                            <p>
                                {isConnected ? 'Your can now accept payments through your application' : 
                                'Connect yout Stripe account to start processing payments and managing subscriptions'}
                            </p>
                        </div>
                    </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-sm text-muted-foreground">
                    {isConnected ? 'You can reconnect anytime if needed' 
                    : 'You will be redirected to Stripe to complete the connection'}
                </div>
                <Link
                    href={stripeLink}
                    className={cn('px-5 py-2.5 rounded-md font-medium text-sm flex items-center gap-2 transition-colors',
            ...
            {!isConnected && (
                <div className='mt-6 pt-6  border-t border-border'>
                    <h3 className='text-sm font-medium mb-2'>Why connect to Stripe ?</h3>
